...
luadir,luapath=...
--luadir,luapath=luajava.luadir.."/buj.aly",luajava.luadir
...
fds_text={
  ...
  "layout_margin","layout_marginLeft","layout_marginTop","layout_marginRight","layout_marginBottom",
  "padding","paddingLeft","paddingTop","paddingRight","paddingButtom","Hint",
}
...
ns={
  "Widget(基础控件)","Check view(检查视图)","Adapter view(适配器)","Advanced Widget(先进部件)","Layout(常用布局)","Advanced Layout(先进的布局)",
"其他控件与布局"
}
...
wds={
  ...
  {"CardView","RadioGroup","GridLayout",
    "ScrollView","HorizontalScrollView","PullingLayout"},

{"LuaEditor","LuaWebView","VideoView"}
}
--汉化table
wds2={
  {"Button(按钮)","EditText(编辑框)","TextView(文本)",
    "ImageButton(图片按钮)","ImageView(图片视图)","CircleImageView(圆图像视图)"},
  
  {"CheckBox(复选框)","RadioButton(单选按钮)","ToggleButton(切换按钮)","Switch(开关)"},
 
  {"ListView(列表视图)","GridView(显示数据表格)","PageView(滑动视图,浏览视图)","ExpandableListView(折叠列表)","Spinner(转轮)"},
 
  {"SeekBar(拖动条)","ProgressBar(进度栏)","RatingBar(评级栏)",
    "DatePicker(日期选择器)","TimePicker(时间选择器)","NumberPicker(数字选择器)"},
 
  {"LinearLayout(线性布局)","AbsoluteLayout(绝对布局)","FrameLayout(框架布局,帧布局)","RelativeLayout(相对布局)"},

  {"CardView(卡片视图)","RadioGroup(选项按键组)","GridLayout(网格布局)",
    "ScrollView(滚动视图)","HorizontalScrollView(横向滚动视图)","PullingLayout(刷新布局)"},

{"LuaEditor(lua代码编辑框)","LuaWebView(lua浏览器)","VideoView(视频控件)"}
}

mAdapter=ArrayExpandableListAdapter(activity)

for k,v in ipairs(ns) do
  mAdapter.add(v,wds2[k])--添加table汉化表
end
